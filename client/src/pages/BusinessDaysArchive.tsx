...
export default function BusinessDaysArchive({
  ...
  return (
    <PageShell
      ...
      ctaTitle={
        language === "en"
          ? "Calculate date intervals with precision"
          : language === "es"
            ? "Calcule intervalos de fechas con precisión"
            : "Calcule intervalos de datas com precisão"
      }
      ctaButtonLabel={
        language === "en"
          ? "Use the calculator"
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_320px]">
            <div className="section-card">
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <article className="rounded-3xl bg-secondary p-5">
                  ...
                  <p className="mt-2 text-2xl font-bold">
                    {monthSummary.weekends}
                  </p>
                </article>
                <article className="rounded-3xl bg-secondary p-5">
                  <p className="text-sm text-muted-foreground">{fifthWord}</p>
                  <p className="mt-2 text-2xl font-bold">
                    {monthSummary.fifthBusinessDay
                      ? formatDate(monthSummary.fifthBusinessDay)
                      : "-"}
                  </p>
                </article>
              </div>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <Link
                  href={`/quinto-dia-util/${year}/${getMonthSlug(activeMonth)}/`}
                  className="rounded-2xl bg-secondary px-4 py-4 font-semibold transition-colors hover:bg-secondary/80"
                >
                  {language === "en"
                    ? "Open the fifth business day page"
                    : language === "es"
                      ? "Abrir la página del quinto día hábil"
                      : "Abrir a página do 5º dia útil"}
                </Link>
              </div>

              <div className="mt-6 space-y-3">
                {monthSummary.holidays.length ? (
                  monthSummary.holidays.map(item => (
                    <div
                      key={item.date}
                      className="rounded-2xl bg-secondary px-4 py-4 text-sm"
                    >
                      <p className="font-semibold">
                        {getHolidayName(item, language)}
                      </p>
                  ...
                  <div className="rounded-2xl bg-secondary px-4 py-4 text-sm text-muted-foreground">
                    {language === "en"
                      ? "No national holidays in this month."
                      : language === "es"
                        ? "No hay feriados nacionales en este mes."
            ...
            <aside className="section-card">
              ...
              <div className="mt-5 space-y-3">
                {shifted ? (
                  <Link
                    href={`/dias-uteis/${shifted.year}/${getMonthSlug(shifted.month)}/`}
                ...
                {shiftedNext ? (
                  <Link
                    href={`/dias-uteis/${shiftedNext.year}/${getMonthSlug(shiftedNext.month)}/`}
                    className="block rounded-2xl bg-secondary px-4 py-4 transition-colors hover:bg-secondary/80"
                  >
                    {language === "en"
                      ? "Next month"
                ...
                {[-1, 1]
                  ...
                  .map(item => (
                    <Link
                      key={item}
                      href={`/dias-uteis/${item}/${getMonthSlug(activeMonth)}/`}
                      className="block rounded-2xl bg-secondary px-4 py-4 transition-colors hover:bg-secondary/80"
                    >
                      {getMonthLabel(item, activeMonth, language, {
                        includeYear: true,
                      })}
                    </Link>
                  ))}
        ...
        <div className="section-card">
          ...
          <div className="mt-5 page-grid">
            <article className="rounded-3xl bg-secondary p-5 text-sm leading-6 text-muted-foreground">
              {language === "en"
                ? "Each year page groups all months in one table with business days, weekends, holidays and the fifth business day."
                : language === "es"
                  ? "Cada página anual reúne todos los meses con días hábiles, fines de semana, feriados y quinto día hábil."
                  : "Cada página anual reúne todos os meses com dias úteis, finais de semana, feriados e 5º dia útil."}
            </article>
            <article className="rounded-3xl bg-secondary p-5 text-sm leading-6 text-muted-foreground">
              {language === "en"
                ? "Monthly pages are useful for deadline checks and for comparing the same month across different years."
                : language === "es"
                  ? "Las páginas mensuales ayudan a revisar plazos y comparar el mismo mes en años distintos."
                  : "As páginas mensais ajudam a revisar prazos e comparar o mesmo mês em anos diferentes."}
            </article>
            <article className="rounded-3xl bg-secondary p-5 text-sm leading-6 text-muted-foreground">
              {language === "en"
                ? "Open past years, the current year and future years whenever you need to compare months."
                : language === "es"
                  ? "Abra años pasados, el actual y años futuros siempre que necesite comparar meses."
                  : "Abra anos passados, o ano atual e anos futuros sempre que precisar comparar meses."}
            </article>
          </div>
        </div>
      </section>
      ...
      <section id="exemplos" className="section-anchor">
        <div className="section-card">
          ...
          <div className="mt-5 page-grid">
            <article className="rounded-3xl bg-secondary p-5 text-sm leading-6 text-muted-foreground">
              {language === "en"
                ? `How many business days are there in ${getMonthLabel(year, month ?? 1, language, { includeYear: true })}?`
                : language === "es"
                  ? `¿Cuántos días hábiles tiene ${getMonthLabel(year, month ?? 1, language, { includeYear: true })}?`
                  : `Quantos dias úteis tem ${getMonthLabel(year, month ?? 1, language, { includeYear: true })}?`}
            </article>
            <article className="rounded-3xl bg-secondary p-5 text-sm leading-6 text-muted-foreground">
              {language === "en"
                ? "Which national holidays affect this month?"
                : language === "es"
                  ? "¿Qué feriados nacionales afectan este mes?"
                  : "Quais feriados nacionais afetam este mês?"}
            </article>
            <article className="rounded-3xl bg-secondary p-5 text-sm leading-6 text-muted-foreground">
              {language === "en"
                ? "What is the fifth business day and how do I compare it with other years?"
                : language === "es"
                  ? "¿Cuál es el quinto día hábil y cómo compararlo con otros años?"
                  : "Qual é o 5º dia útil e como comparar com outros anos?"}
            </article>
          </div>
        </div>
      </section>
